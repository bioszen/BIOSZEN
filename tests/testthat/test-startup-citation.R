...
test_that("direct source launchers emit the BIOSZEN startup citation", {
  ...
  expect_false(
    grepl("cat\\(\"\\\\nBIOSZEN version:[\\s\\S]*?bioszen_startup_citation\\(\\)[\\s\\S]*?# -------- run app --------", standalone_launcher, perl = TRUE),
    info = "Standalone launcher should not print the citation early; the app prints it at the end of startup."
  )
