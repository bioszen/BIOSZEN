...
test_that("global Shiny errors use defensive sanitized session diagnostics", {
  ...
  expect_match(server_txt, 'app_error_recipient <- "bioszenf+bugs@gmail.com"', fixed = TRUE)
  expect_match(server_txt, 'href = paste0("mailto:", app_error_recipient)', fixed = TRUE)
  expect_match(server_txt, "app_error_send_to", fixed = TRUE)
  expect_match(server_txt, "to = app_error_recipient", fixed = TRUE)
